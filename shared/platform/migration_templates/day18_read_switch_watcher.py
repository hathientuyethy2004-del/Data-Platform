...
def update_day18_19_checklist(checklist_path: Path, report: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    should_check_switch = summary_status == "READ_SWITCHED_30" and bool(read_switch.get("applied", False))
    ...
    if changed:
        ...
        return True, "Checklist Day 18-19 updated"

    return False, "No checklist changes needed"


def write_watcher_report(output: Path, checklist_updated: bool, message: str, day18_status: str) -> None:
    now = datetime.now(timezone.utc)
    payload = {
        "run_id": f"day18_watcher_{now.strftime('%Y%m%dT%H%M%SZ')}",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Auto-update Day 18-19 checklist from Day 18 read-switch report")
    parser.add_argument(
        "--report",
    ...
    parser.add_argument(
        "--output",
        default="documentation/migration/reports/DAY18_WATCHER_RESULT.json",
    ...
    day18_status = str(report.get("summary", {}).get("status", "UNKNOWN")).upper()

    changed, message = update_day18_19_checklist(Path(args.checklist), report)
    write_watcher_report(Path(args.output), changed, message, day18_status)

    print(f"[DONE] day18 watcher report: {args.output}")
    print(f"[STATUS] day18_status={day18_status} checklist_updated={changed}")
